...
import { getDesignClass, Flex, Text, Button } from "@vira-ui/ui";
import type { ViraComponentProps } from "@vira-ui/core";
import { useViraContext, resolveModel } from "@vira-ui/core";
import { Input } from "@vira-ui/ui";
import { Calendar, ChevronLeft, ChevronRight } from "lucide-react";

/**
 * ViraDatePicker - DatePicker с поддержкой Vira Framework
 * 
 * Поддерживает:
 * - design prop для стилей
 * - model prop для auto-binding (реактивный)
 * - календарь с навигацией
 * - выбор диапазона дат (опционально)
 * - локализация (опционально)
 */

export interface ViraDatePickerProps
  extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "model" | "type">,
    ViraComponentProps
{
...
export const ViraDatePicker: React.FC<ViraDatePickerProps> = ({
  design,
  preset = "default",
  model,
  modelValue,
  ...
  // Model binding
  const modelBinding = React.useMemo(() => {
  ...
  useEffect(() => {
    if (!model || !modelBinding) return;
    try {
      const service = (modelBinding as any).service;
  ...
  if (modelValue !== undefined) {
    ...
    try {
      computedValue = modelBinding.value || "";
    } catch {
      computedValue = modelBinding.value || "";
    }
  ...
  const handleDateSelect = (day: number) => {
    ...
    if (modelBinding) {
      modelBinding.setValue(formattedDate);
    }
  ...
  return (
    <div className={getDesignClass(wrapperDesign)} data-design={JSON.stringify(wrapperDesign)}>
      <Input
        {...props}
        data-model={typeof model === "string" ? model : undefined}
